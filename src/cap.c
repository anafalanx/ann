 ...
 *
 * `anncap::window <hwnd>` renders a specific window with PrintWindow
 ...
 *
 * Dev-only (C23 + Tcl stubs; links user32 + gdi32). Built by `x build-ext`; NOT
 * compiled into ann.exe.
 */
#include <tcl.h>
...
static int Window_Cmd([[maybe_unused]] void *cd, Tcl_Interp *ip,
    ...
    memcpy(buf, &bih, 40);
    BITMAPINFOHEADER qih = bih;     /* scratch: GetDIBits mutates its BITMAPINFO */
...
int Cap_Init(Tcl_Interp *ip) {
    ...
    Tcl_CreateNamespace(ip, "anncap", nullptr, nullptr);
    Tcl_CreateObjCommand(ip, "anncap::window", Window_Cmd, nullptr, nullptr);
    Tcl_PkgProvide(ip, "anncap", "0.1");
    return TCL_OK;
}
